...
async def process_start_command(message: aiogram.types.Message):
    ...
    if myresult is None or myresult == [] or myresult == ():
        ...
        await message.reply("Siz botni ishga tushirdingiz")
...
async def process_start_command(message: aiogram.types.Message):
    await message.reply("Savolingiz bo`lsa administratorga yozing👉🏻https://t.me/carlosdeamigo")
...
async def process_callback_kb1btn1(callback_query: aiogram.types.CallbackQuery):
    if callback_query.data in cfg.LANGUES:
        ...
        mycursor.execute(sql, val)
        await bot.send_message(callback_query.from_user.id, "Til " + cfg.LANGDICT[lang]+" ga o`zgartirildi"
                                                            "Tarjima qilmoqchi bo`lgan matnni kiriting:")
